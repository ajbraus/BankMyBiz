class MilestonesController < ApplicationController
  ...
  def create
    params[:milestone][:products] = params[:milestone][:products].join(', ')
    if current_user.bank?
      @milestone = current_user.milestones.new(params[:milestone])
    else
      @milestone = current_user.biz_milestones.new(params[:milestone])
    end

    respond_to do |format|
      if @milestone.save
        format.html { redirect_to User.find(@milestone.owner_id), notice: 'Milestone was successfully created.' }
