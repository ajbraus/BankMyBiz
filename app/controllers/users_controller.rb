class UsersController < ApplicationController
  ...
  def index
    if current_user.admin?
      @unconfirmed_users = User.where("confirmed_at IS NULL and rejected_at IS Null").paginate(:page => params[:page], :per_page => 40)
      @confirmed_users = User.where("confirmed_at IS NOT NULL").paginate(:page => params[:page], :per_page => 40, order: "confirmed_at DESC")
      @rejected_users = User.where("rejected_at IS NOT NULL").paginate(:page => params[:page], :per_page => 40)
    else 
